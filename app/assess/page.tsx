...
export default function AssessPage() {
  ...
  const sizeBands = useMemo(
    () => Array.from(new Set(db.companies.map((c) => c.sizeBand))),
    []
  );

  const companyTypes = useMemo(
    () => Array.from(new Set(db.companies.map((c) => c.companyType))),
    []
  );
  ...
  const selected =
    filtered.find((c) => c.id === companyId) ??
    filtered[0] ??
    db.companies[0];
    ...
    <div className="space-y-6">
      <div className="panel grid gap-4 md:grid-cols-5">
        <select
          ...
          className="rounded border border-borderline bg-[#0d1620] px-3 py-2 text-sm"
        >
          {filtered.map((c) => (
      ...
      <div className="panel">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-300">
              {industryObj?.name} • {regionObj?.name} • {selected.sizeBand} •{' '}
          ...
          <Link
            ...
            className="rounded border border-borderline px-3 py-2 text-sm"
          >
            Open detail
          </Link>
        </div>
        ...
        <p className="mt-3 text-xs text-slate-400">
          Prototype note: curated sample company records with simplified assumptions
          for phase 1.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Scorecard
          ...
          score={selected.commercialMaturityScore}
        />
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <MaturityLadder
          score={Math.round(
            (selected.digitalMaturityScore +
              selected.opsMaturityScore +
              selected.commercialMaturityScore) /
              3
          )}
        />
          ...
          {selsignals.map((s) => (
            <div key={s.id} className="rounded border border-borderline p-3 text-sm">
              <div className="font-medium text-white">
                {s.label}{' '}
                <span className="text-xs text-slate-400">({s.strengthLabel})</span>
              </div>
